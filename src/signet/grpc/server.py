...
from . import protected_pb2, protected_pb2_grpc  # type: ignore
...
class ProtectedService(protected_pb2_grpc.ProtectedServicer):  # type: ignore
    async def Call(self, request, context):  # noqa: N802  (gRPC name)
        ...
        if not (sig_input and signature and challenge):
            # Issue challenge via initial metadata
            ...
            return protected_pb2.ProtectedReply(
                ok=False, receipt_id="", pch_verified=False, failure_reason=failure
            )
        ...
        try:
            ...
            presented_nonce = (
                challenge[1:-1]
                if challenge.startswith(":") and challenge.endswith(":")
                else challenge
            )
            nonce_ok = nonce_store.consume(
                route=route, client_ip=client_ip, tls_id="grpc", nonce=presented_nonce
            )
            binding_ok = True  # Simplified for gRPC demo
            ...
            failure = (
                ...
                if verified
                    ...
                    "bad_signature"
                    if not sig_ok
                    else ("nonce_replay" if not nonce_ok else "bad_binding")
                )
            )
        ...
        return protected_pb2.ProtectedReply(
            ...
            pch_verified=verified,
            failure_reason=failure or "",
        )


aSYNC_SERVER_BANNER = "gRPC server listening on"
...
async def serve(port: int = 50051):
    ...
    await server.start()
    print(f"{aSYNC_SERVER_BANNER} {port}")
