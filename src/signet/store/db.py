...
def persist_receipt(rec: Dict[str, Any]):
    ...
    rec_id = rec["id"]
    controller_json = (
        json.dumps(rec.get("controller")) if rec.get("controller") is not None else None
    )
    ...
    with _LOCK:
        ...
        try:
            c.execute(
                "INSERT OR IGNORE INTO receipts(id,ts,decision,reason,controller_json,leaf_hash_b64,batch_id,prev_leaf_hash_b64) VALUES (?,?,?,?,?,?,NULL,?)",
                (
                    rec_id,
                    now,
                    rec["decision"],
...
def _maybe_batch(conn: sqlite3.Connection):
    rows = conn.execute(
        "SELECT id, leaf_hash_b64 FROM receipts WHERE batch_id IS NULL ORDER BY ts"
    ).fetchall()
    if not rows:
        return
    ...
    conn.execute(
        "INSERT INTO batches(created_ts,sth_sig_b64,sth_json,prev_sth_hash_b64) VALUES (?,?,?,?)",
        (int(time.time()), sth_sig_b64, sth_json, prev_sth_hash_b64),
    )
    batch_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.executemany(
        "UPDATE receipts SET batch_id=? WHERE id=?", [(batch_id, rid) for rid, _ in rows]
    )
...
def fetch_receipt(rec_id: str) -> Optional[Dict[str, Any]]:
    with _LOCK:
        ...
        try:
            row = c.execute(
                "SELECT id, ts, decision, reason, controller_json, leaf_hash_b64, batch_id, prev_leaf_hash_b64 FROM receipts WHERE id=?",
                (rec_id,),
            ).fetchone()
            if not row:
                return None
            ...
            if batch_id:
                b = c.execute(
                    "SELECT sth_json, sth_sig_b64 FROM batches WHERE id=?", (batch_id,)
                ).fetchone()
                if b:
